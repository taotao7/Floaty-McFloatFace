...
fn tray_text(locale: &str) -> (&'static str, &'static str, &'static str, &'static str) {
    match locale {
        "zh-CN" => ("显示/隐藏摄像头窗", "打开设置", "锁定/解锁拖拽", "退出"),
        _ => ("Show/Hide Camera", "Open Settings", "Lock/Unlock Drag", "Quit"),
    }
}

fn build_tray_menu(app: &AppHandle, locale: &str) -> Result<Menu<tauri::Wry>, Box<dyn std::error::Error>> {
    let (show_text, settings_text, lock_text, quit_text) = tray_text(locale);
    ...
    let menu = Menu::with_items(
        app,
        &[&show_toggle, &open_settings, &toggle_lock, &separator, &quit],
    )?;
